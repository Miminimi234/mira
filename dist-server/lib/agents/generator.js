        ...
        try {
            ...
            for (let i = 0; i < selectedMarkets.length; i++) {
                ...
                if (shouldAttemptTrades) {
                    ...
                    try {
                        const already = await persistence.marketHasTrade?.(scored.id);
                        if (already) {
                    ...
                    try {
                        ...
                        if (trade) {
                            ...
                            try {
                                await persistence.saveTrade?.(trade);
                                console.log(`[Agent:${agentId}] 🔖 Marked market ${trade.marketId} as predicted in persistence`);
                            }
                            catch (err) {
                                console.warn(`[Agent:${agentId}] ⚠️ Failed to persist trade for market ${trade.marketId}:`, err.message);
                            }
                        }
